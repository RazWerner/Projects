...
@max_score(.3)
def test_subset_runner(test_name, width_p, depth_log2_p):
    parameters = {"width_p": width_p,
                  "depth_log2_p": depth_log2_p}

    simulator = os.getenv('SIM').lower()
    # Note how the run/build paths change with parameterized tests.
    run_dir = os.path.join(_DIR_PATH, "run", simulator, get_param_string({"test_name":test_name, **parameters}))
